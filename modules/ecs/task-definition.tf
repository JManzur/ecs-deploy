...
resource "null_resource" "push" {
  ...
  provisioner "local-exec" {
    ...
    command     = "${coalesce("${path.root}/scripts/push.sh")} ${var.aws_region} ${var.aws_profile} ${path.module}/docker/demo-app ${aws_ecr_repository.demo_flask_app.repository_url} latest"
    interpreter = ["bash", "-c"]
  }
}
...
data "template_file" "demo_app_tpl" {
  ...
  vars = {
    ...
    app_image     = aws_ecr_repository.demo_flask_app.repository_url
